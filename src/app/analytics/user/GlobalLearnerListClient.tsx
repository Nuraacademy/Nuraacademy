...
export default function GlobalLearnerListClient({ learners }: GlobalLearnerListClientProps) {
    ...
    const filteredLearners = learners.filter(learner =>
        ...
        learner.classTitle.toLowerCase().includes(searchValue.toLowerCase()) ||
        ...
        <main className="min-h-screen bg-[#F9F9EE] text-[#1C3A37]">
            <div className="max-w-7xl mx-auto px-6 md:px-10 py-8">
                ...
                <TitleCard
                    ...
                    description="Global Learner Report List"
                />

                <div className="bg-white rounded-xl p-8 md:p-12 shadow-sm border border-[#1C3A37]/5 space-y-8">
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                        <h2 className="text-lg font-medium text-[#1C3A37] tracking-tight border-l-4 border-[#DAEE49] pl-4 uppercase">All Enrolled Learners</h2>
                        <NuraSearchInput
                                ...
                                {filteredLearners.length === 0 && (
                                    <tr>
                                        <td colSpan={3} className="px-6 py-12 text-center text-gray-400 italic">
                                            {searchValue ? "No matching learners found." : "No enrolled learners found in any class."}
                                        </td>
                                    </tr>
                                )}
                            </tbody>
