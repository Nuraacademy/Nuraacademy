    ...
    if (evaluator.id === evaluatee.id) {
        ...
        return (
            <div className="flex flex-col items-center justify-center min-h-screen bg-[#F9F9EE] p-8 text-center">
                <h1 className="text-2xl font-medium text-[#1C3A37] mb-4">Self-Feedback Not Required</h1>
                <p className="text-gray-600 mb-8">You cannot provide peer feedback to yourself.</p>
                <button onClick={() => redirect(`/classes/${evaluatee.classId}/analytics`)} className="bg-[#DAEE49] text-[#1C3A37] px-8 py-3 rounded-full font-medium">Back to Report</button>
            </div>
        );
    }
