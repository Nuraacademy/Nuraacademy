        ...
        <div className="min-h-screen bg-[#F9F9EE] px-4 md:px-16 py-8 md:py-12 space-y-8 font-sans">
            ...
            <div className="bg-white rounded-[40px] p-8 md:p-12 shadow-sm border border-white/50 space-y-12">
                ...
                <div className="space-y-8">
                    <h2 className="text-lg font-bold text-[#1C3A37]">Peer Feedback Summary</h2>
                    <div className="space-y-4 max-w-4xl">
                        {metrics.map((m) => (
                            <div key={m.label} className="flex items-center gap-6">
                                <span className="text-[10px] font-bold text-gray-500 uppercase w-32">{m.label}</span>
                                {feedback ? (
                                    <>
                                        ...
                                        <span className="text-[10px] font-bold text-gray-400 w-4 text-right">{m.score}</span>
                                    </>
                                ) : (
                                    <span className="text-[10px] font-bold text-gray-300 italic tracking-wider">Feedback not available</span>
                                )}
                            </div>
                        ))}
                    </div>
                ...
                <div className="space-y-8 pt-8 border-t border-gray-100">
                    <h2 className="text-lg font-bold text-[#1C3A37]">Feedback Details</h2>
                        ...
                        {feedbackDetails.map((detail) => {
                            ...
                            return (
                                <div key={detail.label} className="space-y-4 relative">
                                    <h3 className="text-sm font-bold text-[#1C3A37]">{detail.label}</h3>
                                    <div 
