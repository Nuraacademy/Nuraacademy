                ...
                {trainers.slice(0, 3).map((trainer, idx) => (
                    <React.Fragment key={trainer.id}>
                        <div className="flex items-center gap-5 group cursor-pointer">
                            <div className="relative w-14 h-14 rounded-full overflow-hidden shadow-sm group-hover:shadow-md transition-all border-2 border-white">
                                {trainer.image ? (
                                    <Image
                                        src={trainer.image}
                                        alt={trainer.name}
                            ...
                            <div>
                                <h4 className="text-md font-medium text-[#1C3A37] group-hover:text-[#2D5A54] transition-colors">{trainer.name}</h4>
                                <p className="text-sm text-gray-400 font-medium">{trainer.role}</p>
                            </div>
                        </div>
